...
public class GuildsGuildModule implements GuildModule {
    ...
    public AbstractGuild getGuild(PlayerData playerData) {
        final var guild = Guilds.getApi().getGuild(playerData.getUniqueId());
        return guild == null ? null : new CustomGuild(guild);
    }

    @Override
    public @NotNull Relationship getRelationship(Player player, Player target) {
        final var guild1 = Guilds.getApi().getGuild(player);
        if (guild1 == null) return Relationship.GUILD_NEUTRAL;

        final var guild2 = Guilds.getApi().getGuild(target);
        if (guild2 == null) return Relationship.GUILD_NEUTRAL;
